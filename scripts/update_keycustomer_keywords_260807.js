...
// 最新 Excel 文件路径 (260807)
const EXCEL_PATH = '/Users/tristan/Downloads/企业名称缩写对照表（更新关联关键词）-260807.xlsx';
...
async function updateKeyCustomerKeyWords() {
  ...
  for (let i = 1; i < rawRows.length; i++) {
    ...
    // A列 GID (索引 0)
    const gid = row[0] != null ? String(row[0]).trim() : '';
    const nameEn = row[1] != null ? String(row[1]).trim() : '';
    const nameCn = row[2] != null ? String(row[2]).trim() : '';
    // J列 关键字合集 (索引 9)
    const rawKeyWords = row[9] != null ? String(row[9]).trim() : '';
