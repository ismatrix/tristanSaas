...
(function () {
    ...
    // 清理 Token 值中的 Bearer 前缀并解析可能的 JSON
    function cleanTokenValue(val) {
        if (!val) return null;
        try {
            if (val.startsWith('{')) {
                let parsed = JSON.parse(val);
                val = parsed.v || parsed.token || parsed.accessToken || val;
            }
        } catch (e) { }
        if (typeof val !== 'string') return null;
        val = val.replace(/bearer /i, '').trim();
        if (val.length > 20) {
            return val;
        }
        return null;
    ...
    // 尝试从不同的存储位置提取 SCM Token
    function getScmTokenFromStorage() {
        const possibleKeys = [
            'access_token', 'token', 'Authorization', 'authToken', 'scm_token',
            'user_token', 'jwt_token', 'session_token', 'scm_manual_token', 'scm_auto_token'
        ];

        // 1. 直接尝试这几个常见的键名
        for (const key of possibleKeys) {
            let val = localStorage.getItem(key) || sessionStorage.getItem(key);
            let clean = cleanTokenValue(val);
            if (clean) return clean;
        }

        // 2. 遍历所有的 storage 键寻找可能匹配的 Token
        try {
        ...
        try {
            ...
            for (let c of cookies) {
                c = c.trim();
                if (c.toLowerCase().startsWith('token=') || c.toLowerCase().startsWith('authorization=')) {
                    let val = c.substring(c.indexOf('=') + 1);
                    let clean = cleanTokenValue(val);
                    if (clean) return clean;
                }
            }
        } catch (e) {
            console.error('解析 Cookie 出错:', e);
        }

        return null;
    }

    // 综合获取 SCM Token，依次尝试内存变量、自动拦截缓存和 Storage 检索
    function getScmToken() {
        if (scmToken) return scmToken;
        let autoToken = localStorage.getItem('scm_auto_token');
        if (autoToken) return autoToken;
        let storageToken = getScmTokenFromStorage();
    ...
    // 专门获取并验证 SCM Token，如果获取不到则弹窗输入并缓存
    const ensureScmToken = async function () {
        let token = getScmToken();
        if (!token) {
            const manualToken = prompt(
                '⚠️ 未自动获取到 SCM 认证 Token。\n请在控制台 Network 中找到 listCompanySupplier 的 Authorization 请求头，或在此处输入 Token (支持 Bearer xxx 格式)：',
                localStorage.getItem('scm_manual_token') || ''
            );
            if (manualToken === null) {
                console.log('用户取消了 Token 输入。');
                return null;
            }
            token = manualToken.replace(/bearer /i, '').trim();
            if (!token) {
                alert('输入的 Token 不能为空！');
                return null;
            }
            localStorage.setItem('scm_manual_token', token);
        }
        scmToken = token;
        return token;
    };
        ...
        try {
            ...
            while (page < totalPages) {
                ...
                if (records.length > 0) {
                    ...
                    console.log(`✅ 第 ${page + 1} 页获取并插入数据库成功 | 耗时: ${Date.now() - t0}ms | 本次新增: ${records.length} 条`);
                }
                page++;
            }
    ...
    // --- 5.1 获取单个参与方详情 ---
    window.runParticipantDetailQuery = function (companyBasicId) {
        return new Promise((resolve, reject) => {
            const token = getScmToken();
            if (!token) {
                reject('未获取到 Token');
                return;
            }
            ...
            const url = `https://scm.cmi.chinamobile.com:8443/spfm/v1/16/participant-access/query/${companyBasicId}`;
    ...
    // --- 5.2 并发批量获取全部参与方详情并回写 ---
    const runAllParticipantsDetailsQuery = async function () {
        let token = await ensureScmToken();
        if (!token) return;

        console.log('📡 正在从本地数据库获取已有的参与方列表...');
        let dbRecords = [];
        try {
            dbRecords = await new Promise((resolve, reject) => {
                const url = 'http://127.0.0.1:3000/v1/wildcards/ibossParticipants?projection={"companyBasicId":1}&options={"limit":0}';
                GM_xmlhttpRequest({
                    method: "GET",
                    url: url,
        ...
        let ids = dbRecords.map(r => r.companyBasicId).filter(id => id !== undefined && id !== null && id !== '');
        ids = [...new Set(ids)];

        if (ids.length === 0) {
            alert('数据库中未找到任何有效的 companyBasicId 记录。');
            return;
        }

        console.log(`📊 共找到 ${ids.length} 个参与方 BasicId，开始分批并发获取详情...`);
        ...
        for (let i = 0; i < ids.length; i += BATCH_SIZE) {
            ...
            if (validDetails.length > 0) {
                await new Promise((resolve) => {
                    GM_xmlhttpRequest({
                        method: "POST",
                        url: "http://127.0.0.1:3000/v1/wildcards/ibossParticipantDetail/bulk-upsert",
                        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${adminToken}` },
    ...
    const injectParticipantUI = () => {
        ...
        b5.innerText = '🔄 同步参与方'; // 优化按钮文案并添加表情符号
        b5.style.cssText = btnStyle + 'background: #6f42c1;';
        b5.onclick = runParticipantQuery;

        const b6 = document.createElement('button');
        b6.innerText = '📋 获取全部参与方详情';
    ...
    // --- 8. 拦截与路由分发 ---
    // 扩大判定范围：同时支持 URL 包含 eipid=756 的顶层页面，或者 scm.cmitry.com 的 iframe 页面
    const isParticipantPage = location.href.includes('eipid=756') || location.hostname.includes('scm.cmitry.com');
    ...
    if (isParticipantPage) {
        ...
        window.addEventListener('message', function (event) {
            if (event.data && event.data.type === 'SCM_TOKEN') {
                ...
                // 缓存自动截获的 Token，以便后续刷新页面或直接点击时直接使用
                localStorage.setItem('scm_auto_token', scmToken);
            }
        });

        // 无论是否拦截到 Token，都定期检查并注入「同步参与方」按钮
        setInterval(injectParticipantUI, 2000);
    } else {
        // 其他常规页面
